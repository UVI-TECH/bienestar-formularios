...
interface Props {
  /**
   * Enfermera preseleccionada, cuando coincide con la persona en sesión (ver
   * `nombreDeSesionEnCatalogo`). El campo sigue siendo un desplegable normal.
   */
  enfermeraPorDefecto?: string;
}
...
export default function AtencionPorAccidente({
  ...
  const datosIniciales = useMemo(
    () => ({ ...DATOS_VACIOS, enfermera: enfermeraPorDefecto }),
    [enfermeraPorDefecto],
  );
  const [datos, setDatos] = useState<DatosPoliza>(datosIniciales);
  ...
  const validar = useCallback((): ErroresFormulario => {
    const encontrados = limpiarErrores({
      ...
      facultad: datos.facultad ? undefined : "Seleccione la facultad.",
