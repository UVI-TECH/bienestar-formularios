...
/**
 * Lista de asistentes ya agregados a la actividad, con opción de eliminar
 * cada uno antes de guardar. Cada fila entra animada; bajo
 * `prefers-reduced-motion` aparece y desaparece sin desplazamiento.
 */
    ...
    >
      <div className="min-w-0 flex-1">
        <p className="text-ayuda font-medium text-texto">
          {asistente.nombresApellidos}
        </p>
        <p className="mt-0.5 font-mono text-meta text-texto-tenue">
          {[asistente.documento, asistente.dependencia].filter(Boolean).join(" · ")}
        </p>
      </div>
      ...
      <button
        type="button"
        onClick={onEliminar}
        aria-label={`Eliminar a ${asistente.nombresApellidos} de la lista`}
