...
interface Props {
  /**
   * Auxiliar preseleccionada, cuando coincide con la persona en sesión (ver
   * `nombreDeSesionEnCatalogo`). El campo sigue siendo un desplegable normal.
   */
  auxiliarPorDefecto?: string;
}

export default function Tamizaje({ auxiliarPorDefecto = "" }: Props) {
  const datosIniciales = useMemo(
    () => ({ ...DATOS_VACIOS, auxiliar: auxiliarPorDefecto }),
    [auxiliarPorDefecto],
  );
  const [datos, setDatos] = useState<DatosTamizaje>(datosIniciales);
  ...
  const aplicarConsulta = useCallback((traidos: RespuestaConsultaCedula) => {
    ...
    setErrores((previos) =>
      sinErrores(previos, "nombres", "apellidos", "programa", "facultad"),
    );
